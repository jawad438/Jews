...
const App: React.FC = () => {
  ...
  const resetGame = useCallback(() => {
    ...
    setGameObjects([]);
    setPlayerPosition({
      x: (gameArea.width - PLAYER_SIZE) / 2,
      y: gameArea.height - PLAYER_SIZE - 10,
    });
  ...
  const gameLoop = useCallback((currentTime: number) => {
    ...
    setPlayerPosition(prev => {
        ...
        return {
            x: Math.max(0, Math.min(bounds.width - PLAYER_SIZE, newX)),
            y: prev.y,
        };
    });
  ...
  return (
    <div className="min-h-screen w-full bg-white flex font-sans">
      {(() => {
        let src = '';
        if (gameState === GameState.Playing) {
            src = 'https://open.spotify.com/embed/track/6Wa2uRtYLoo4idpCWmGUq4?utm_source=generator&autoplay=1';
        } else if (gameState === GameState.GameOver) {
            src = 'https://open.spotify.com/embed/track/6iUVUzg0c2O0KJjxdOvrdr?utm_source=generator&autoplay=1';
        }
      ...
      <div className="w-16 md:w-24 bg-[#1635B9] flex-shrink-0"></div>
      <div className="flex-grow flex flex-col items-center justify-center p-2 sm:p-4">
        <div className="w-full max-w-4xl">
          ...
          <main 
              ref={gameAreaRef} 
              className="w-full aspect-[4/3] bg-white/50 backdrop-blur-sm border-2 border-blue-900 rounded-lg overflow-hidden relative shadow-2xl"
          >
              {getModal()}
              {gameState === GameState.Playing && (
                  <>
                      ...
                      {gameObjects.map(obj => (
                          <img
                              key={obj.id}
                              src={obj.type === 'coin' ? COIN_IMAGE_URL : ENEMY_IMAGE_URL}
                              alt={obj.type}
                              className="absolute"
                              style={{
                                  width: OBJECT_SIZE,
